...
if (isset($_POST['nuevo_invitado'])) {
    $nombres = array_map('trim', $_POST['nombres']);
    $codigo = generarCodigo();

    $stmt = $pdo->prepare("INSERT INTO invitados (codigo, nombre) VALUES (:codigo, :nombre)");
...
if (isset($_POST['editar_invitado'])) {
    ...
    $stmt = $pdo->prepare("SELECT id FROM invitados WHERE codigo = :codigo ORDER BY id");
    $stmt->execute([':codigo' => $codigo]);
    $ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

    foreach ($ids as $index => $id) {
        if (isset($nombres[$index])) {
            $stmtUpdate = $pdo->prepare("UPDATE invitados SET nombre = :nombre, asistencia = :asistencia WHERE id = :id");
            $stmtUpdate->execute([
...
if (isset($_GET['eliminar'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM invitados WHERE codigo = :codigo");
    $stmt->execute([':codigo' => $codigo]);
    header("Location: admin.php");
    exit;
}
...
// --- LEER DATOS ---
$stmt = $pdo->query("SELECT * FROM invitados ORDER BY codigo, id");
